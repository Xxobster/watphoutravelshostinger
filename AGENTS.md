...
Rebuild **watphou-travels.com** from Wix to WordPress with a custom block theme and two custom plugins. Staging runs on Hostinger at **https://watphoutravels.site** (send this URL to reviewers). The Hostinger preview name `https://darkslategray-snake-182151.hostingersite.com` is a fallback only: some visitor networks return NXDOMAIN for `*.hostingersite.com`. The old Virtual Private Server (VPS) demo `watphou.smbistro.duckdns.org` on `sm` was removed on 2026-09-10. Production stays on Hostinger when the real domain is attached. Never touch `smbistro.duckdns.org` or other sites on VPS `sm`. Never change live Wix / Joker Domain Name System (DNS) for `watphou-travels.com` until cutover.
...
7. Do not deploy to Virtual Private Server (VPS) `sm`. Verify staging with `python scripts/verify_demo.py`. Deploy theme/plugins to Hostinger, then confirm `https://watphoutravels.site`.
